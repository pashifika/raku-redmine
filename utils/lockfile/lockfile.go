...
var (
	...
	ErrNeedAbsPath   = errors.New("lockfile must be given as absolute path names")
...
// Please note, that existing lockfile containing pids of dead processes
...
func makePidFile(name string, pid int) (tmpName string, cleanup func(), err error) {
	tmpl, err := os.CreateTemp(filepath.Dir(name), filepath.Base(name)+".")
	if err != nil {
		return "", nil, err
	}
	...
	cleanup = func() {
		_ = tmpl.Close()
		_ = os.Remove(tmpl.Name())
	}

	if _, err := io.WriteString(tmpl, fmt.Sprintf("%d\n", pid)); err != nil {
		cleanup() // Do cleanup here, so call doesn't have to.
		return "", nil, err
	}

	return tmpl.Name(), cleanup, nil
}
