...
func (c *Client) CreateIssue(issue Issue) (*Issue, error) {
	...
	req, err := http.NewRequest("POST", c.endpoint+"/issues.json?key="+c.apikey, strings.NewReader(conv.BytesToString(s)))
	if err != nil {
		return nil, err
	}
...
func (c *Client) UpdateIssue(issue Issue) error {
	...
	req, err := http.NewRequest("PUT", c.endpoint+"/issues/"+strconv.Itoa(issue.Id)+".json?key="+c.apikey, strings.NewReader(conv.BytesToString(s)))
	if err != nil {
		return err
	}
	req.Header.Set("Content-Type", "application/json")
